...
readonly WORKSPACE="${XYCAR_WORKSPACE:-$REPOSITORY_ROOT/ros2_ws}"
...
if [[ ! -f "$WORKSPACE/install/setup.bash" ]]; then
  echo "ROS 2 workspace has not been built: $WORKSPACE" >&2
  echo 'Run: source /opt/ros/humble/setup.bash && cd ros2_ws && colcon build --symlink-install' >&2
  exit 1
fi
...
if [[ "$EXECUTION_BACKEND" == container ]]; then
  docker image inspect "$CONTAINER_IMAGE" >/dev/null 2>&1 || {
    ...
    echo 'Provide the team image with XYCAR_IMAGE, or use XYCAR_EXECUTION_BACKEND=host.' >&2
    exit 1
  }
fi
