...
if [[ $# -lt 1 || $# -gt 3 ]]; then
  echo "usage: $0 RUN_DIR [DURATION_S] [off|profile]" >&2
  exit 2
fi
...
if [[ ! -c /dev/video0 ]]; then
  echo '/dev/video0 is not available' >&2
  exit 3
fi
if fuser /dev/video0 >/dev/null 2>&1; then
  echo '/dev/video0 is already in use; no process was stopped' >&2
  exit 3
fi
mkdir -p "$RUN_DIR"
...
ros2 launch mission_cone_drive integrated_drive.launch.py \
  start_camera:=true start_lidar:=false start_ultrasonic:=false \
  start_yolo:=true start_lane_stack:=true start_rviz:=false \
  start_active:=false use_sim_time:=false \
...
v4l2-ctl --device /dev/video0 --get-fmt-video --get-parm \
  >"$RUN_DIR/camera_runtime.txt" 2>&1 || true
...
ros2 bag record --storage sqlite3 --storage-preset-profile resilient \
  ...
  /lane_yolo/detections /center_curve \
...
while (( elapsed < DURATION_S )); do
  step=5
  if (( DURATION_S - elapsed < step )); then
    step=$((DURATION_S - elapsed))
  fi
  sleep "$step"
  elapsed=$((elapsed + step))
  echo "live freshness test progress: ${elapsed}/${DURATION_S}s"
done
...
if fuser /dev/video0 >/dev/null 2>&1; then
  echo 'warning: /dev/video0 is still busy after cleanup' >&2
fi
printf 'completed=%s\n' "$(date --iso-8601=seconds)" >"$RUN_DIR/run_complete.txt"
echo "motor-safe live USB freshness run completed: $RUN_DIR"
