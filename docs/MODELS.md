...
최종 통합 실행에서 직접 선택되는 파일은 다음 두 개이다.
...
최종 후보 비교와 회귀 확인에 사용한 대회용 체크포인트도 함께 보존했다.
...
모든 파일은 `ros2_ws/src/cam/cam/`에 있다. 저장소 용량을 줄이기 위해 이 시스템과 관계없는 YOLOv8 medium detection/pose/segmentation 데모 가중치는 제외했다.

체크포인트는 실행 재현을 위한 산출물이다. 학습 데이터셋은 개인정보·용량·배포 범위 문제로 포함하지 않는다.
